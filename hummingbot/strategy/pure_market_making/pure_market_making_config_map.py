...
def maker_trading_pair_prompt():
    ...
    return _(f"Enter the token trading pair you would like to trade on {exchange}{example} >>> ").format(
        exchange=exchange, example=f" (e.g. {example})" if example else "")
...
def validate_price_source(value: str) -> Optional[str]:
    if value not in {"current_market", "external_market", "custom_api"}:
        ...
        return _("Invalid price source type.")
...
def price_source_market_prompt() -> str:
    ...
    return _("Enter the token trading pair on {external_market} >>> ").format(external_market=external_market)
...
def validate_price_source_exchange(value: str) -> Optional[str]:
    if value == pure_market_making_config_map.get("exchange").value:
        ...
        return _("Price source exchange cannot be the same as maker exchange.")
...
def validate_price_floor_ceiling(value: str) -> Optional[str]:
    ...
    if not (decimal_value == Decimal("-1") or decimal_value > Decimal("0")):
        ...
        return _("Value must be more than 0 or -1 to disable this feature.")
...
def validate_decimal_list(value: str) -> Optional[str]:
    ...
    for number in decimal_list:
        try:
            ...
            return _("Please enter valid decimal numbers")
...
pure_market_making_config_map = {
    ...
    "exchange":
        ConfigVar(key="exchange",
                  prompt=_("Enter your maker spot connector >>> "),
    ...
    "bid_spread":
        ConfigVar(key="bid_spread",
                  prompt=_("How far away from the mid price do you want to place the "
    ...
    "minimum_spread":
        ConfigVar(key="minimum_spread",
                  prompt=_("At what minimum spread should the bot automatically cancel orders? (Enter 1 for 1%) >>> "),
    ...
    "max_order_age":
        ConfigVar(key="max_order_age",
                  prompt=_("How long do you want to cancel and replace bids and asks "
    ...
    "price_ceiling":
        ConfigVar(key="price_ceiling",
                  prompt=_("Enter the price point above which only sell orders will be placed "
    ...
    "price_floor":
        ConfigVar(key="price_floor",
                  prompt=_("Enter the price below which only buy orders will be placed "
                           "(Enter -1 to deactivate this feature) >>> "),
                  type_str="decimal",
                  default=Decimal("-1"),
                  validator=validate_price_floor_ceiling),
    "moving_price_band_enabled":
        ConfigVar(key="moving_price_band_enabled",
                  prompt=_("Would you like to enable moving price floor and ceiling? (Yes/No) >>> "),
                  type_str="bool",
                  default=False,
    ...
    "price_ceiling_pct":
        ConfigVar(key="price_ceiling_pct",
                  prompt=_(
                      "Enter a percentage to the current price that sets the price ceiling. Above this price, only sell orders will be placed >>> "),
                  type_str="decimal",
                  default=Decimal("1"),
    ...
    "price_floor_pct":
        ConfigVar(key="price_floor_pct",
                  prompt=_(
                      "Enter a percentage to the current price that sets the price floor. Below this price, only buy orders will be placed >>> "),
                  type_str="decimal",
                  default=Decimal("-1"),
                  required_if=lambda: pure_market_making_config_map.get("moving_price_band_enabled").value,
                  validator=validate_decimal),
    "price_band_refresh_time":
        ConfigVar(key="price_band_refresh_time",
                  prompt=_(
                      "After this amount of time (in seconds), the price bands are reset based on the current price >>> "),
                  type_str="float",
                  default=86400,
                  required_if=lambda: pure_market_making_config_map.get("moving_price_band_enabled").value,
                  validator=validate_decimal),
    "ping_pong_enabled":
        ConfigVar(key="ping_pong_enabled",
                  prompt=_(
                      "Would you like to use the ping pong feature and alternate between buy and sell orders after fills? (Yes/No) >>> "),
                  type_str="bool",
                  default=False,
                  prompt_on_new=True,
                  validator=validate_bool),
    "order_levels":
        ConfigVar(key="order_levels",
                  prompt=_("How many orders do you want to place on both sides? >>> "),
                  type_str="int",
                  validator=lambda v: validate_int(v, min_value=-1, inclusive=False),
                  default=1),
    "order_level_amount":
        ConfigVar(key="order_level_amount",
                  prompt=_("How much do you want to increase or decrease the order size for each "
    ...
    "order_level_spread":
        ConfigVar(key="order_level_spread",
                  prompt=_("Enter the price increments (as percentage) for subsequent "
    ...
    "inventory_target_base_pct":
        ConfigVar(key="inventory_target_base_pct",
                  prompt=_("What is your target base asset percentage? Enter 50 for 50% >>> "),
    ...
    "inventory_price":
        ConfigVar(key="inventory_price",
                  prompt=_("What is the price of your base asset inventory? "),
                  type_str="decimal",
    ...
    "filled_order_delay":
        ConfigVar(key="filled_order_delay",
                  prompt=_("How long do you want to wait before placing the next order "
    ...
    "hanging_orders_enabled":
        ConfigVar(key="hanging_orders_enabled",
                  prompt=_("Do you want to enable hanging orders? (Yes/No) >>> "),
                  type_str="bool",
                  default=False,
                  validator=validate_bool),
    "hanging_orders_cancel_pct":
        ConfigVar(key="hanging_orders_cancel_pct",
                  prompt=_("At what spread percentage (from mid price) will hanging orders be canceled? "
                           "(Enter 1 to indicate 1%) >>> "),
    ...
    "order_optimization_enabled":
        ConfigVar(key="order_optimization_enabled",
                  prompt=_("Do you want to enable best bid ask jumping? (Yes/No) >>> "),
                  type_str="bool",
                  default=False,
                  validator=validate_bool),
    "ask_order_optimization_depth":
        ConfigVar(key="ask_order_optimization_depth",
                  prompt=_("How deep do you want to go into the order book for calculating "
    ...
    "add_transaction_costs":
        ConfigVar(key="add_transaction_costs",
                  prompt=_("Do you want to add transaction costs automatically to order prices? (Yes/No) >>> "),
                  type_str="bool",
                  default=False,
                  validator=validate_bool),
    "price_source":
        ConfigVar(key="price_source",
                  prompt=_("Which price source to use? (current_market/external_market/custom_api) >>> "),
                  type_str="str",
    ...
    "price_type":
        ConfigVar(key="price_type",
                  prompt=_("Which price type to use? ("
    ...
    "price_source_exchange":
        ConfigVar(key="price_source_exchange",
                  prompt=_("Enter external price source exchange name >>> "),
    ...
    "take_if_crossed":
        ConfigVar(key="take_if_crossed",
                  prompt=_("Do you want to take the best order if orders cross the orderbook? ((Yes/No) >>> "),
    ...
    "price_source_custom_api":
        ConfigVar(key="price_source_custom_api",
                  prompt=_("Enter pricing API URL >>> "),
                  required_if=lambda: pure_market_making_config_map.get("price_source").value == "custom_api",
                  type_str="str"),
    "custom_api_update_interval":
        ConfigVar(key="custom_api_update_interval",
                  prompt=_("Enter custom API update interval in second (default: 5.0, min: 0.5) >>> "),
                  required_if=lambda: False,
                  default=float(5),
                  type_str="float",
    ...
    "should_wait_order_cancel_confirmation":
        ConfigVar(key="should_wait_order_cancel_confirmation",
                  prompt=_("Should the strategy wait to receive a confirmation for orders cancelation "
    ...
    "split_order_levels_enabled":
        ConfigVar(key="split_order_levels_enabled",
                  prompt=_("Do you want bid and ask orders to be placed at multiple defined spread and amount? "
    ...
    "bid_order_level_spreads":
        ConfigVar(key="bid_order_level_spreads",
                  prompt=_("Enter the spreads (as percentage) for all bid spreads "
    ...
    "ask_order_level_spreads":
        ConfigVar(key="ask_order_level_spreads",
                  prompt=_("Enter the spreads (as percentage) for all ask spreads "
                           "e.g 1,2,3,4 to represent 1%,2%,3%,4%. "
                           "The number of levels set will be equal to the "
                           "minimum length of bid_order_level_spreads and bid_order_level_amounts >>> "),
                  default=None,
    ...
    "bid_order_level_amounts":
        ConfigVar(key="bid_order_level_amounts",
                  prompt=_("Enter the amount for all bid amounts. "
                           "e.g 1,2,3,4. "
                           "The number of levels set will be equal to the "
                           "minimum length of bid_order_level_spreads and bid_order_level_amounts >>> "),
                  default=None,
    ...
    "ask_order_level_amounts":
        ConfigVar(key="ask_order_level_amounts",
                  prompt=_("Enter the amount for all ask amounts. "
                           "e.g 1,2,3,4. "
                           "The number of levels set will be equal to the "
                           "minimum length of bid_order_level_spreads and bid_order_level_amounts >>> "),
                  default=None,
