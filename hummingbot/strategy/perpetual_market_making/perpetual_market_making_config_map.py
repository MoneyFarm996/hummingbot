...
def maker_trading_pair_prompt():
    ...
    return _("Enter the token trading pair you would like to trade on %s%s >>> ") % (
        derivative, f" (e.g. {example})" if example else "")
...
def validate_derivative_position_mode(value: str) -> Optional[str]:
    if value not in ["One-way", "Hedge"]:
        return _("Position mode can either be One-way or Hedge mode")
...
def validate_price_source_derivative(value: str) -> Optional[str]:
    if value == perpetual_market_making_config_map.get("derivative").value:
        ...
        return _("Price source derivative cannot be the same as maker derivative.")
    if validate_derivative(value) is not None and validate_exchange(value) is not None:
        ...
        return _("Price source must must be a valid exchange or derivative connector.")
...
perpetual_market_making_config_map = {
    ...
    "derivative":
        ConfigVar(key="derivative",
                  prompt=_("Enter your maker derivative connector exchange name >>> "),
    ...
    "leverage":
        ConfigVar(key="leverage",
                  prompt=_("How much leverage do you want to use? "
    ...
    "position_mode":
        ConfigVar(key="position_mode",
                  prompt=_("Which position mode do you want to use? (One-way/Hedge) >>> "),
    ...
    "short_profit_taking_spread":
        ConfigVar(key="short_profit_taking_spread",
                  prompt=_(
                      "At what spread from the position entry price do you want to place a long order to reduce position? (Enter 1 for 1%) >>> "),
                  type_str="decimal",
                  default=Decimal("0"),
                  validator=lambda v: validate_decimal(v, 0, 100, True),
                  prompt_on_new=True),
    ...
    "stop_loss_slippage_buffer":
        ConfigVar(key="stop_loss_slippage_buffer",
                  prompt=_(
                      "How much buffer should be added in stop loss orders' price to account for slippage? (Enter 1 for 1%)? >>> "),
                  type_str="decimal",
                  default=Decimal("0.5"),
    ...
    "price_source_derivative":
        ConfigVar(key="price_source_derivative",
                  prompt=_("Enter external price source connector name or derivative name >>> "),
